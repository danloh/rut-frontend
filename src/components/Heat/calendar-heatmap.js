...
export let calendarHeatmap = {
  ...
  // defaults
    ...
    function chart () {
      ...
      let monthRange = d3.time.months(moment(yearAgo).startOf('month').add(1, 'month').toDate(), now) // it ignores the first month
      ...
      // color range
      let color = d3.scale.linear().range(chart.colorRange()).domain([0, max])
      ...
      function drawChart () {
        ...
        if (chart.tooltipEnabled()) {
          dayRects.on('mouseover', function (d, i) {
            ...
            .style('left', function () { return (Math.floor(i / 7) + 4) * SQUARE_LENGTH + 'px' })
            .style('top', function () {
      ...
      function pluralizedTooltipUnit (count) {
        if (typeof tooltipUnit === 'string') {
          return ( tooltipUnit ? tooltipUnit + (count === 1 ? '' : 's') : '')
        }
      ...
      function tooltipHTMLForDate (d) {
        let dateStr = moment(d).format('MMM D, YYYY')
